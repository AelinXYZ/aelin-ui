...
const GasSelector: React.FC<IGasSelector> = ({
	...
	const formattedGasPrice = useMemo(() => {
		...
		if (!nCustomGasPrice) {
			if (!Number.isInteger(nGasPrice)) {
				return nGasPrice.toFixed(isOVM ? 3 : 2);
			}
			return nGasPrice;
		}

		if (!Number.isInteger(nCustomGasPrice)) {
			return nCustomGasPrice.toFixed(isOVM ? 3 : 2);
		}
