...
const Airdrop = () => {
	...
	useEffect(() => {
		const getGasLimitEstimate = async () => {
			if (!walletAddress || !merkleBalance || !canClaimMerkle || !aelinDistributionContract) {
				return;
			}
			try {
	...
	useEffect(() => {
		const getGasLimitEstimate = async () => {
			if (!walletAddress || !airdropBalance || !canClaim || !vAelinDistributionContract) {
				return;
			}
			try {
	...
	const handlevAelinClaim = useCallback(async () => {
		if (isSubmitButtonDisabled || !gasvAelinLimitEstimate || !vAelinDistributionContract) {
			return;
		}
		try {
