...
export const useGetMintDealTokensById = <K extends keyof MintDealTokensResult>(url: string, options?: SingleQueryOptions, args?: MintDealTokensArgs<K>, queryOptions: UseQueryOptions<Pick<MintDealTokensResult, K>> = {}) => {
    const func = async function <K extends keyof MintDealTokensResult>(url: string, options: SingleQueryOptions, args: MintDealTokensArgs<K>): Promise<Pick<MintDealTokensResult, K>> {
        const res = await axios.post(url, {
    ...
    const enabled = options && args;
    return useQuery(["codegen-graphql", enabled ? generateGql("MintDealTokens", options, args) : null], async () => func(url, options!, args!), {
        ...queryOptions,
        enabled: !!options && !!args,
    });
};
export const useGetMintDealTokenss = <K extends keyof MintDealTokensResult>(url: string, options?: MultiQueryOptions<MintDealTokensFilter, MintDealTokensResult>, args?: MintDealTokensArgs<K>, queryOptions: UseQueryOptions<Pick<MintDealTokensResult, K>[]> = {}) => {
    const func = async function <K extends keyof MintDealTokensResult>(url: string, options: MultiQueryOptions<MintDealTokensFilter, MintDealTokensResult>, args: MintDealTokensArgs<K>): Promise<Pick<MintDealTokensResult, K>[]> {
        const paginatedOptions: Partial<MultiQueryOptions<MintDealTokensFilter, MintDealTokensResult>> = { ...options };
        ...
        if (options.first && options.first > MAX_PAGE) {
            ...
            paginatedOptions.orderBy = options.orderBy || "id";
            paginatedOptions.orderDirection = options.orderDirection || "asc";
            paginationKey = paginatedOptions.orderBy + (paginatedOptions.orderDirection === "asc" ? "_gt" : "_lt") as keyof MintDealTokensFilter;
            paginatedOptions.where = { ...options.where };
        }
    ...
    const enabled = options && args;
    return useQuery(["codegen-graphql", enabled ? generateGql("MintDealTokenss", options, args) : null], async () => func(url, options!, args!), {
        ...queryOptions,
        enabled: !!options && !!args,
    });
};
